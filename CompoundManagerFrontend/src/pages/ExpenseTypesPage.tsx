...
export default function ExpenseTypesPage() {
  ...
  function load() {
    api.getExpenseTypes(true).then(setTypes).catch(console.error);
  }
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    try {
      ...
      setName('');
      setMessage('تم إضافة نوع المصروف');
      load();
    } catch (err) {
  ...
  async function handleUpdate(id: number) {
    setError('');
    try {
      await api.updateExpenseType(id, { name: editName.trim() });
      setEditingId(null);
  ...
  async function handleToggle(id: number) {
    await api.toggleExpenseType(id);
    load();
  }

  async function handleDelete(id: number) {
    if (!confirm('حذف نوع المصروف؟')) return;
    setError('');
    try {
  ...
  return (
    <div>
      <PageHeader title="أنواع المصاريف" />
          ...
          <form onSubmit={handleCreate} className="flex flex-wrap items-end gap-3">
            <FormField label="اسم النوع" className="mb-0 min-w-[200px] flex-1">
              <Input
                ...
                placeholder="مثال: صيانة، كهرباء، نظافة..."
                required
                maxLength={50}
              />
            </FormField>
      ...
      <Card>
        <CardContent className="pt-6">
          ...
          {types.length === 0 && <EmptyState>لا توجد أنواع مصاريف</EmptyState>}
        </CardContent>
      </Card>
    </div>
  );
