...
export default function ServiceTypesScreen() {
  ...
  async function save() {
    ...
    try {
      if (editingId) {
        await api.updateServiceType(editingId, { name: typeName.trim() });
        Alert.alert('تم بنجاح', 'تم تعديل نوع الخدمة بنجاح.');
      } else {
        await api.createServiceType(typeName.trim());
        Alert.alert('تم بنجاح', 'تم إضافة نوع الخدمة بنجاح.');
      }
  ...
  async function triggerDelete() {
    ...
    setDeleting(true);
    try {
      await api.deleteServiceType(deleteTarget.id);
      Alert.alert('تم بنجاح', 'تم حذف نوع الخدمة بنجاح.');
  ...
  async function triggerToggle() {
    ...
    setToggling(true);
    try {
      await api.toggleServiceType(toggleTarget.id);
      Alert.alert(
        'تم بنجاح',
        `تم ${toggleTarget.activeFlag === 'Y' ? 'إيقاف' : 'تفعيل'} نوع الخدمة بنجاح.`
      );
  ...
  return (
    ...
    >
      ...
      <View style={styles.subHeader}>
        <Text style={styles.pageTitle}>أنواع الخدمات</Text>
        {isStaff && (
        ...
        <View style={styles.tableContainer}>
          ...
          {types.length === 0 ? (
            <View style={styles.emptyView}>
              <Text style={styles.emptyText}>لا توجد أنواع خدمات مسجلة</Text>
            </View>
          ) : (
            types.map((item, idx) => {
